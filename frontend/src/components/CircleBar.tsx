...
function CircleBar({heightSVG ,widthSVG, workouts, target}:IProps) {
  ...
  const percentage = (workouts/target)
  ...
  return (
      <svg width={widthSVG} height={heightSVG} className="fill-[#FAFAFA] font-bold">
        <g >
          <path transform={transformValue} d={backgroundArc} fill="#ddd" />
          <path transform={transformValue} d={foregroundArc} fill={`url(#gradient-${percentage})`} />
